# eflob Connect Clerk Setup

eflob Connect uses one Clerk application for web, desktop, and mobile authentication. The relay accepts
Clerk JWTs only when they are generated from the `eflob-relay` template with the shared
`eflob-relay` audience.

## Application Keys

eflob Connect is disabled in a fresh clone. To enable it for source builds, add a repository-root `.env`
or `.env.local` file:

```dotenv
EFLOB_CLERK_PUBLISHABLE_KEY=<publishable key>
EFLOB_CLERK_JWT_TEMPLATE=<JWT template name>
EFLOB_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
EFLOB_RELAY_URL=https://relay.example.com
```
...
should set `EFLOB_CLERK_PUBLISHABLE_KEY`, `EFLOB_CLERK_JWT_TEMPLATE`,
`EFLOB_CLERK_CLI_OAUTH_CLIENT_ID`, and `EFLOB_RELAY_URL` before building. EAS preview and
...
deployment defaults.
`vp run --filter eflob-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
5. Set `EFLOB_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
| ------- | ---------------------------- |
| Name    | `eflob-relay`                   |
| Claims  | `{ "aud": "eflob-relay" }` |

Set `EFLOB_CLERK_JWT_TEMPLATE=eflob-relay` in the repository-root `.env`, and set
`CLERK_JWT_AUDIENCE=eflob-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `EFLOB_RELAY_URL` still
...
Local desktop development uses `eflob-dev://app`, while packaged builds use `eflob://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `eflob-dev://app`; the production Clerk instance should only need
`eflob://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `com.eflob.eflob`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.eflob.eflob` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.eflob.eflob`.
...
```dotenv
EFLOB_APPLE_TEAM_ID=ABC1234567
EFLOB_MACOS_PROVISIONING_PROFILE=/absolute/path/to/eflob.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
EFLOB_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `EFLOB_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`EFLOB_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `EFLOB_PORT` set. Rebuild the signed app
...
EFLOB_PORT=13773 \
  "/Applications/eflob (Alpha).app/Contents/MacOS/eflob (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/eflob (Alpha).app"
codesign -d --entitlements :- "/Applications/eflob (Alpha).app"
```
...
Approved signed-in users manage eflob Connect under **Connections**. The web and desktop sidebars do
not expose a dedicated account or waitlist control. Signed-out users reach Clerk's waitlist and
sign-in flow contextually from the eflob Connect controls on the Connections page.
