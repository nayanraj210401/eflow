...
This guide is for people who want to use more than one Codex account in eflob.
...
In eflob Settings, name it something obvious:
...
In eflob Settings, add another Codex provider:
...
eflob shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, eflob treats it
