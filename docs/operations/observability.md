...
eflob has one server-side observability model:
...
```bash
export EFLOB_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export EFLOB_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export EFLOB_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export EFLOB_TRACE_MIN_LEVEL=Info
export EFLOB_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `EFLOB_OTLP_*`.

macOS app bundle example:

```bash
EFLOB_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
EFLOB_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
EFLOB_OTLP_SERVICE_NAME=t3-desktop \
"/Applications/eflob.app/Contents/MacOS/eflob"
```
...
```bash
TRACE_FILE="${EFLOB_HOME:-$HOME/.t3}/userdata/logs/server.trace.ndjson"
```
...
- `EFLOB_OTLP_TRACES_URL` was not set
...
- `EFLOB_TRACE_FILE`: override trace file path
- `EFLOB_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `EFLOB_TRACE_MAX_FILES`: rotated file count, default `10`
- `EFLOB_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `EFLOB_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `EFLOB_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `EFLOB_OTLP_TRACES_URL`: OTLP trace endpoint
- `EFLOB_OTLP_METRICS_URL`: OTLP metric endpoint
- `EFLOB_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `EFLOB_OTLP_SERVICE_NAME`: service name, default `t3-server`
