...
- `eflob-relay-traces-prod`, an OpenTelemetry trace dataset for Worker requests
- `eflob-relay-otel-ingest-prod`, a dataset-scoped ingest token bound to the Worker
- `eflob-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`eflob-relay-traces-dev-julius` for a personal stage.
