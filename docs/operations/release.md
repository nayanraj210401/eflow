...
```sh
vp run --filter eflob-relay deploy -- --stage "$USER" --env-file .env.local
```
...
- `EFLOB_WEB_ROUTER_URL`: defaults to `https://app.eflob.dev`.
- `EFLOB_WEB_LATEST_DOMAIN`: defaults to `latest.app.eflob.dev`.
- `EFLOB_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.eflob.dev`.

Required Vercel domains:

- `app.eflob.dev`: the router domain users open, updated by stable releases.
- `latest.app.eflob.dev`: channel alias updated by stable releases.
- `nightly.app.eflob.dev`: channel alias updated by nightly releases.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__eflob/channel?channel=latest` or
`/__eflob/channel?channel=nightly`; the router stores the
`eflob_web_channel` cookie and rewrites future requests on `app.eflob.dev` to
...
through `/__eflob/channel` on the router domain so the user's channel cookie is
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `app.eflob.dev` points at a deployment containing the router
...
- Repository slug source:
  - `EFLOB_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `EFLOB_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
2. Create an explicit App ID for `com.eflob.eflob` and enable Associated Domains.
...
10. Complete the Clerk Native API and AASA setup in [eflob Connect Clerk Setup](../cloud/eflob-connect-clerk.md#desktop-passkeys).
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.com.eflob.eflob` and includes
    Associated Domains.
