...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.EFLOB_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EFLOB_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EFLOB_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EFLOB_RELAY_URL).toBeUndefined();
    expect(env.VITE_EFLOB_RELAY_URL).toBeUndefined();
    expect(env.EFLOB_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.EFLOB_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.EFLOB_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.EFLOB_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.EFLOB_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.EFLOB_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "EFLOB_CLERK_PUBLISHABLE_KEY=pk_root\nEFLOB_CLERK_JWT_TEMPLATE=template_root\nEFLOB_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nEFLOB_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "EFLOB_CLERK_PUBLISHABLE_KEY=pk_local\nEFLOB_CLERK_JWT_TEMPLATE=template_local\nEFLOB_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nEFLOB_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).EFLOB_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          EFLOB_CLERK_PUBLISHABLE_KEY: "pk_ci",
          EFLOB_CLERK_JWT_TEMPLATE: "template_ci",
          EFLOB_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          EFLOB_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      EFLOB_RELAY_URL: "https://ci.example.test",
      VITE_EFLOB_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        EFLOB_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_EFLOB_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          EFLOB_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          EFLOB_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          EFLOB_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
  ...
  it("projects canonical mobile tracing values to Expo public aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          EFLOB_RELAY_URL: "https://relay.example.test",
          EFLOB_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          EFLOB_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          EFLOB_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      EFLOB_RELAY_URL: "https://relay.example.test",
      VITE_EFLOB_RELAY_URL: "https://relay.example.test",
      EFLOB_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      EFLOB_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      EFLOB_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "eflob-public-config-"));
