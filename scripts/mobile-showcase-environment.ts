...
export const SHOWCASE_PROJECT_ID = "eflob";
...
export const SHOWCASE_TERMINAL_BUFFER = [
  "\u001b[38;5;75m~/Code/eflob\u001b[0m \u001b[38;5;212mfeat/remote-command-center\u001b[0m",
  "$ vp test run --changed",
  "",
  "  \u001b[38;5;117meflob-mobile\u001b[0m       184 passed",
...
const PROJECT_FAVICONS = {
  eflob: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
...
export const SHOWCASE_PROJECTS = [
  {
    ...
    title: "eflob",
    directory: "eflob",
    repositoryUrl: "https://github.com/yourorg/eflob.git",
    favicon: PROJECT_FAVICONS.eflob,
  },
  {
    id: "react",
    title: "React",
...
async function seedeflobWorkspace(workspaceRoot: string): Promise<void> {
  ...
  await NodeFSP.writeFile(
    ...
    `${JSON.stringify({ name: "eflob", private: true, scripts: { test: "vp test" } }, null, 2)}\n`,
  );
  await NodeFSP.writeFile(NodePath.join(workspaceRoot, "favicon.svg"), PROJECT_FAVICONS.eflob);
  await NodeFSP.writeFile(
...
async function seedCompanionWorkspace(input: {
  ...
  await NodeFSP.writeFile(
    ...
    `# ${input.title}\n\nSeeded by the eflob mobile screenshot harness.\n`,
  );
  await initializeRepository({
...
export async function seedShowcaseEnvironment(input: {
  ...
  if (primaryProject.id === SHOWCASE_PROJECT_ID) {
    await seedeflobWorkspace(workspaceRoot);
  }
  await Promise.all(
    projects
