...
const ANDROID_PACKAGE = "com.eflob.eflob.dev";
const APP_SCHEME = "eflob-dev";
...
const IOS_APP_PATH = NodePath.join(
  MOBILE_ROOT,
  ".showcase/ios-derived-data/Build/Products/Debug-iphonesimulator/eflobDev.app",
);
...
Usage:
  pnpm --filter @eflob/mobile screenshots [options]
...
async function buildIos(): Promise<string> {
  ...
  await runCommand(
    ...
    [
      "-workspace",
      NodePath.join(MOBILE_ROOT, "ios/eflobDev.xcworkspace"),
      "-scheme",
      "eflobDev",
      "-configuration",
      "Debug",
