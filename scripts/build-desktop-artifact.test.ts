...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "eflob (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "eflob (Nightly)");
  });
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                EFLOB_DESKTOP_UPDATE_REPOSITORY: "yourorg/eflob",
              },
            }),
          ),
        ),
      ...
      const nightlyConfig = yield* resolveGitHubPublishConfig("nightly").pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                GITHUB_REPOSITORY: "yourorg/eflob",
              },
            }),
          ),
        ),
      ...
      resolveDesktopRuntimeDependencies(
        {
          ...
          "@eflob/contracts": "workspace:*",
          "@eflob/shared": "workspace:*",
          "@eflob/ssh": "workspace:*",
          "@eflob/tailscale": "workspace:*",
          effect: "catalog:",
          electron: "41.5.0",
        },
        {
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      ...
      EFLOB_MACOS_PROVISIONING_PROFILE: "/tmp/eflob.provisionprofile",
      EFLOB_CLERK_PUBLISHABLE_KEY: `pk_test_${btoa("example.clerk.accounts.dev$")}`,
    });

    assert.deepStrictEqual(configuration, {
      appId: "com.eflob.eflob",
      teamId: "ABC1234567",
      rpDomains: ["example.clerk.accounts.dev"],
      provisioningProfilePath: "/tmp/eflob.provisionprofile",
    });
  });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      EFLOB_APPLE_TEAM_ID: "ABC1234567",
      EFLOB_MACOS_PROVISIONING_PROFILE: "/tmp/eflob.provisionprofile",
      EFLOB_CLERK_PASSKEY_RP_DOMAINS:
    ...
    assert.include(entitlements, "<string>ABC1234567.com.eflob.eflob</string>");
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    const missingProfileError = captureError({
      EFLOB_APPLE_TEAM_ID: "ABC1234567",
      EFLOB_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev",
    });
    assert.instanceOf(missingProfileError, MissingMacPasskeyProvisioningProfileError);
    assert.equal(
      missingProfileError.message,
      "EFLOB_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.",
    );
    ...
    const invalidDomainError = captureError({
      EFLOB_APPLE_TEAM_ID: "ABC1234567",
      EFLOB_MACOS_PROVISIONING_PROFILE: "/tmp/eflob.provisionprofile",
      EFLOB_CLERK_PASSKEY_RP_DOMAINS: unsafeDomain,
    });
    ...
    assert.throws(
      () =>
        resolveMacPasskeySigningConfiguration({
          EFLOB_APPLE_TEAM_ID: "ABC1234567",
          EFLOB_MACOS_PROVISIONING_PROFILE: "/tmp/eflob.provisionprofile",
          EFLOB_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev:8443",
        }),
    ...
    const invalidPublishableKeyError = captureError({
      EFLOB_APPLE_TEAM_ID: "ABC1234567",
      EFLOB_MACOS_PROVISIONING_PROFILE: "/tmp/eflob.provisionprofile",
      EFLOB_CLERK_PUBLISHABLE_KEY: "pk_test_%",
    });
    ...
    assert.equal(invalidPublishableKeyError.message, "EFLOB_CLERK_PUBLISHABLE_KEY is invalid.");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.eflob.eflob");
      assert.equal(mac.entitlements, "/tmp/entitlements.mac.plist");
      assert.equal(mac.provisioningProfile, "/tmp/eflob.provisionprofile");
      assert.deepStrictEqual(mac.protocols, [
        { name: "eflob", schemes: ["eflob", "eflob-dev"] },
      ]);
      ...
      const resolved = yield* resolveBuildOptions({
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                EFLOB_DESKTOP_SKIP_BUILD: "true",
                EFLOB_DESKTOP_KEEP_STAGE: "true",
                EFLOB_DESKTOP_SIGNED: "true",
                EFLOB_DESKTOP_VERBOSE: "true",
                EFLOB_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
