...
import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@eflob/contracts";
...
describe("buildHomeThreadGroups", () => {
  it("builds one v2 scope for the same repository across environments", () => {
    ...
    const repositoryIdentity = {
      canonicalKey: "github.com/yourorg/eflob",
      locator: {
        source: "git-remote" as const,
        remoteName: "origin",
        remoteUrl: "git@github.com:yourorg/eflob.git",
      },
    };
    const projects = [
      makeProject({
    ...
    expect(scopes[0]?.title).toBe("eflob");
  ...
  it("routes stale duplicate project refs through the canonical repository group", () => {
    ...
    const local = makeProject({
      ...
      environmentId: localEnvironmentId,
      title: "eflob",
      workspaceRoot: "/workspaces/eflob",
      repositoryIdentity,
    });
  ...
  it("matches web repository, repository-path, and separate grouping modes", () => {
    const environmentId = EnvironmentId.make("environment-1");
    const repositoryIdentity = {
      canonicalKey: "github.com/eflob/eflob",
      locator: {
        source: "git-remote" as const,
        remoteName: "origin",
        remoteUrl: "git@github.com:eflob/eflob.git",
      },
      provider: "github",
      owner: "eflob",
      name: "eflob",
      displayName: "eflob",
      rootPath: "/workspaces/eflob",
    };
    const projects = [
      makeProject({
        environmentId,
        ...
        title: "Web",
        workspaceRoot: "/workspaces/eflob/apps/web",
        repositoryIdentity,
      }),
      makeProject({
        environmentId,
        ...
        title: "Mobile",
        workspaceRoot: "/workspaces/eflob/apps/mobile",
        repositoryIdentity,
      }),
    ];
    const threads = projects.map((project) =>
  ...
  it("does not apply the recency window while searching", () => {
    ...
    const group = buildGroups([project], threads, { searchQuery: "eflob" })[0];
