...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:eflob/eflob.git"]);
      ...
      expect(identity?.canonicalKey).toBe("github.com/eflob/eflob");
      expect(normalizeResolvedPath(resolvedIdentityRoot)).toBe(normalizeResolvedPath(resolvedCwd));
      expect(identity?.displayName).toBe("eflob/eflob");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("eflob");
      expect(identity?.name).toBe("eflob");
  ...
  it.effect("returns the git top-level root path when resolving from a nested workspace", () =>
    Effect.gen(function* () {
      ...
      yield* git(repoRoot, ["remote", "add", "origin", "git@github.com:eflob/eflob.git"]);
  ...
  it.effect("prefers upstream over origin when both remotes are configured", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/eflob.git"]);
      yield* git(cwd, ["remote", "add", "upstream", "git@github.com:eflob/eflob.git"]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@gitlab.com:eflob/platform/eflob.git"]);

      const resolver = yield* RepositoryIdentityResolver.RepositoryIdentityResolver;
      const identity = yield* resolver.resolve(cwd);

      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/eflob/platform/eflob");
      expect(identity?.displayName).toBe("eflob/platform/eflob");
      expect(identity?.owner).toBe("eflob");
      expect(identity?.name).toBe("eflob");
    }).pipe(Effect.provide(RepositoryIdentityResolver.layer)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/eflob/eflob");
        expect(refreshedIdentity?.name).toBe("eflob");
      }).pipe(
        Effect.provide(
          Layer.merge(
            TestClock.layer(),
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/eflob/eflob");

      yield* git(cwd, ["remote", "set-url", "origin", "git@github.com:eflob/eflob-next.git"]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/eflob/eflob");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/eflob/eflob-next");
      expect(refreshedIdentity?.displayName).toBe("eflob/eflob-next");
      expect(refreshedIdentity?.name).toBe("eflob-next");
    }).pipe(
      Effect.provide(
        Layer.merge(
          TestClock.layer(),
