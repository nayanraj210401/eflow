...
const bitbucketPullRequest = {
  ...
  links: {
    html: {
      href: "https://bitbucket.org/yourorg/eflob/pull-requests/42",
    },
  },
  source: {
    branch: { name: "feature/source-control" },
    repository: {
      full_name: "octocat/eflob",
  ...
  destination: {
    branch: { name: "main" },
    repository: {
      full_name: "yourorg/eflob",
      workspace: { slug: "yourorg" },
    },
  },
};
...
const repositoryJson = {
  full_name: "yourorg/eflob",
  links: {
    html: { href: "https://bitbucket.org/yourorg/eflob" },
    clone: [
      { name: "https", href: "https://bitbucket.org/yourorg/eflob.git" },
      { name: "ssh", href: "git@bitbucket.org:yourorg/eflob.git" },
    ],
  },
...
function makeLayer(input: {
  ...
  const gitMock = {
    readConfigValue: vi.fn<GitVcsDriver.GitVcsDriver["Service"]["readConfigValue"]>(() =>
      Effect.succeed<string | null>("git@bitbucket.org:yourorg/eflob.git"),
    ),
    ...
    listRemotes: () =>
      Effect.succeed({
        remotes: [
          {
            name: "origin",
            url: "git@bitbucket.org:yourorg/eflob.git",
            pushUrl: Option.none(),
            isPrimary: true,
          },
        ],
    ...
    Layer.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            EFLOB_BITBUCKET_API_BASE_URL: "https://api.test.local/2.0",
            EFLOB_BITBUCKET_EMAIL: "user@example.com",
            EFLOB_BITBUCKET_API_TOKEN: "token",
          },
        }),
      ),
    ),
...
it.effect("parses pull request responses from the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(result, {
      number: 42,
      title: "Add Bitbucket provider",
      url: "https://bitbucket.org/yourorg/eflob/pull-requests/42",
      baseRefName: "main",
    ...
    assert.strictEqual(
      ...
      "https://api.test.local/2.0/repositories/yourorg/eflob/pullrequests/42",
    );
...
it.effect("lists pull requests with Bitbucket state and source branch query params", () => {
        ...
        values: [
          {
            ...
            source: {
              ...
              repository: { full_name: "yourorg/eflob" },
            },
          },
        ],
      }),
  ...
  return Effect.gen(function* () {
    ...
    assert.strictEqual(
      request?.url,
      "https://api.test.local/2.0/repositories/yourorg/eflob/pullrequests",
    );
...
it.effect("reads repository clone URLs and default branch", () => {
  ...
  return Effect.gen(function* () {
    ...
    const cloneUrls = yield* bitbucket.getRepositoryCloneUrls({
      cwd: "/repo",
      repository: "yourorg/eflob",
    });
    const defaultBranch = yield* bitbucket.getDefaultBranch({ cwd: "/repo" });

    assert.deepStrictEqual(cloneUrls, {
      nameWithOwner: "yourorg/eflob",
      url: "https://bitbucket.org/yourorg/eflob.git",
      sshUrl: "git@bitbucket.org:yourorg/eflob.git",
    });
  ...
  () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        [
          "https://api.test.local/2.0/repositories/yourorg/eflob",
          "https://api.test.local/2.0/repositories/yourorg/eflob/branching-model",
        ].toSorted(),
      );
    }).pipe(Effect.provide(layer));
  },
...
it.effect("creates repositories through the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    const request = execute.mock.calls[0]?.[0];
    assert.strictEqual(request?.url, "https://api.test.local/2.0/repositories/yourorg/eflob");
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(JSON.parse(new TextDecoder().decode(rawBody)), {
      ...
      source: {
        ...
        repository: { full_name: "owner/eflob" },
      },
      destination: {
        branch: { name: "main" },
      },
  ...
  return Effect.gen(function* () {
    ...
    yield* bitbucket.checkoutPullRequest({
      ...
      context: {
        ...
        remoteName: "origin",
        remoteUrl: "git@bitbucket.org:yourorg/eflob.git",
      },
      reference: "42",
      force: true,
    });
...
it.effect("checks out fork pull requests through an ensured fork remote", () => {
  const { git, layer } = makeLayer({
    response: (request) => {
      if (request.url.endsWith("/repositories/octocat/eflob")) {
        return Response.json({
          ...repositoryJson,
          full_name: "octocat/eflob",
          links: {
            html: { href: "https://bitbucket.org/octocat/eflob" },
            clone: [
              { name: "https", href: "https://bitbucket.org/octocat/eflob.git" },
              { name: "ssh", href: "git@bitbucket.org:octocat/eflob.git" },
            ],
          },
        });
      }
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(git.ensureRemote.mock.calls[0]?.[0], {
      ...
      url: "git@bitbucket.org:octocat/eflob.git",
    });
    assert.deepStrictEqual(git.fetchRemoteBranch.mock.calls[0]?.[0], {
      cwd: "/repo",
      remoteName: "octocat",
      remoteBranch: "main",
      localBranch: "eflob/pr-42/main",
    });
    assert.deepStrictEqual(git.setBranchUpstream.mock.calls[0]?.[0], {
      cwd: "/repo",
      branch: "eflob/pr-42/main",
      remoteName: "octocat",
      remoteBranch: "main",
    });
    assert.deepStrictEqual(git.switchRef.mock.calls[0]?.[0], {
      cwd: "/repo",
      refName: "eflob/pr-42/main",
    });
  }).pipe(Effect.provide(layer));
});
