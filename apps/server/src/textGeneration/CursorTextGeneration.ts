...
import { type CursorSettings, type ModelSelection } from "@eflob/contracts";
import { sanitizeBranchFragment, sanitizeFeatureBranchName } from "@eflob/shared/git";
import { extractJsonObject } from "@eflob/shared/schemaJson";

import { TextGenerationError } from "@eflob/contracts";
  ...
  const runCursorJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      ...
      const runtime = yield* makeCursorAcpRuntime({
        ...
        cwd,
        clientInfo: { name: "eflob-git-text", version: "0.0.0" },
