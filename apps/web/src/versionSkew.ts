import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@eflob/contracts";
...
export const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "eflob:version-mismatch-dismissals:v1";
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same eflob version.",
  };
}
...
export function serverUpdateGuidance(
  ...
  switch (capability) {
    ...
    case "desktop-managed":
      return `The ${serverLabel} is run by the eflob desktop app on its machine — update the desktop app there to sync them.`;
    default:
