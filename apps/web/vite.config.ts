...
const configuredRelayUrl = repoEnv.VITE_EFLOB_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.EFLOB_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: EFLOB_BUNDLED_DEV=1 pnpm dev:web
const bundledDevEnv = process.env.EFLOB_BUNDLED_DEV?.trim().toLowerCase();
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_EFLOB_RELAY_URL": JSON.stringify(configuredRelayUrl),
