...
const shouldLaunchElectronAfterPack = process.env.EFLOB_DESKTOP_DEV === "1";
  ...
  run: {
    tasks: {
      build: {
        ...
        dependsOn: ["eflob#build"],
        cache: false,
      },
      dev: {
        command:
          "node scripts/build-preview-annotation-css.mjs && cross-env EFLOB_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["eflob#build"],
        cache: false,
      },
      "dev:bundle": {
  ...
  pack: [
    {
      ...
      clean: true,
      deps: {
        alwaysBundle: (id) => id.startsWith("@eflob/"),
      },
