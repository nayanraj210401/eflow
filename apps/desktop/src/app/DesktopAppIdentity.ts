...
const AppPackageMetadata = Schema.Struct({
  eflobCommitHash: Schema.optional(Schema.String),
});
...
>()("@eflob/desktop/app/DesktopAppIdentity") {}
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.eflobCommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),
