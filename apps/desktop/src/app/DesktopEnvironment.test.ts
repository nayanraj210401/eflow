    ...
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          ...
          EFLOB_DEV_REMOTE_T3_SERVER_ENTRY_PATH: " /remote/server.mjs ",
          EFLOB_OTLP_TRACES_URL: " http://127.0.0.1:4318/v1/traces ",
          EFLOB_OTLP_EXPORT_INTERVAL_MS: "2500",
        },
      );
      ...
      assert.equal(environment.appUserModelId, "com.eflob.eflob.dev");
      assert.equal(environment.linuxWmClass, "eflob-dev");
      assert.deepEqual(
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          EFLOB_DESKTOP_APP_USER_MODEL_ID: " com.eflob.eflob.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.eflob.eflob.dev.local");
    }),
  );
