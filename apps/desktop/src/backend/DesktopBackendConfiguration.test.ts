    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          EFLOB_PORT: "9999",
          EFLOB_MODE: "desktop",
          EFLOB_DESKTOP_LAN_HOST: "192.168.1.50",
...
describe("DesktopBackendConfiguration", () => {
  it.effect("resolvePrimary produces a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.EFLOB_PORT);
        assert.isUndefined(first.env.EFLOB_MODE);
        assert.isUndefined(first.env.EFLOB_DESKTOP_LAN_HOST);
  ...
  it("resolvePrimaryLabel is runSync-safe against the real WSL availability probe", async () => {
    ...
    // oxlint-disable-next-line eflob/no-manual-effect-runtime-in-tests -- This test intentionally replicates the sync IPC handler's runSync path to catch a regression to async-only resolution; it.effect would mask it.
    ...
    try {
      ...
      // oxlint-disable-next-line eflob/no-manual-effect-runtime-in-tests -- Same reason: this is the synchronous resolution the IPC handler performs.
