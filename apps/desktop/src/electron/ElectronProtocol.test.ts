...
describe("ElectronProtocol", () => {
  ...
  it.effect("proxies the stable renderer origin to the current app server", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* protocol.registerDesktopProtocol({
            ...
            clerkFrontendApiHostname: "clerk.eflob.dev",
          });
          assert.isDefined(handler);

          const response = yield* Effect.promise(() =>
            handler!(
              new Request("eflob-dev://app/api/health?verbose=1", {
                headers: {
                  accept: "application/json",
                  origin: "eflob-dev://app",
                  referer: "eflob-dev://app/",
          ...
          assert.include(
            ...
            "script-src 'self' 'unsafe-inline' https://clerk.eflob.dev https://challenges.cloudflare.com",
          );
          assert.include(
            response.headers.get("content-security-policy") ?? "",
          ...
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "img-src 'self' eflob-dev: blob: data: http: https:",
          );
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "font-src 'self' eflob-dev: data:",
          );
        }),
      );
      ...
      assert.deepEqual(unhandleMock.mock.calls, [["eflob-dev"]]);
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("eflob://other/")));
        }),
      );
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("eflob-dev://app/")));
        }),
      );
  ...
  it.effect("preserves protocol registration failures", () =>
    Effect.gen(function* () {
      ...
      assert.equal(error.scheme, "eflob-dev");
      assert.strictEqual(error.cause, cause);
      assert.equal(error.message, 'Failed to register Electron protocol scheme "eflob-dev".');
    }).pipe(Effect.provide(ElectronProtocol.layer)),
  );
  ...
  it.effect("preserves protocol unregistration failures", () =>
    Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(error.scheme, "eflob");
        assert.strictEqual(error.cause, cause);
        assert.equal(error.message, 'Failed to unregister Electron protocol scheme "eflob".');
      }
    }).pipe(Effect.provide(ElectronProtocol.layer)),
  );
  ...
  it("keeps executable sources host-restricted while allowing runtime network resources", () => {
    ...
    assert.deepEqual(directives["font-src"], ["'self'", "eflob:", "data:"]);
  });
});
