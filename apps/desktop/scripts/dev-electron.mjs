...
const remoteDebuggingPort = process.env.EFLOB_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line eflob/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.EFLOB_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.EFLOB_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--eflob-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--eflob-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
