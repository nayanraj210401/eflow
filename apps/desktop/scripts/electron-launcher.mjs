// This file mostly exists because we want dev mode to say "eflob (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "eflob (Dev)" : "eflob (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.eflob.eflob.dev.${devBundleIdSuffix || "local"}`
  : "com.eflob.eflob";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["eflob-dev"] : ["eflob"];
...
// oxlint-disable-next-line eflob/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["EFLOB_PORT", environment.EFLOB_PORT],
    ["EFLOB_HOME", environment.EFLOB_HOME],
    ["EFLOB_COMMIT_HASH", environment.EFLOB_COMMIT_HASH],
    ["EFLOB_OTLP_TRACES_URL", environment.EFLOB_OTLP_TRACES_URL],
    ["EFLOB_OTLP_EXPORT_INTERVAL_MS", environment.EFLOB_OTLP_EXPORT_INTERVAL_MS],
    ["EFLOB_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --eflob-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the eflob name.
