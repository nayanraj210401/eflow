...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("eflob-relay-traces", "prod")).toBe(
      "eflob-relay-traces-prod",
    );
    expect(relayResourceNameForStage("eflob-relay-traces", "dev_julius")).toBe(
      "eflob-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "eflobrelay.com", hash)).toBe(
      "prod-abcdef0123456789.eflobrelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "eflobrelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
