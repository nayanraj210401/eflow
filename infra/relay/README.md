...
The relay is intentionally not in the hot path for normal eflob traffic. After a client connects,
regular API and WebSocket traffic goes directly between that client and the selected environment.
See the [T3 Connect architecture overview](../../docs/cloud/eflob-connect-auth-flow.html) for the larger system
design.
...
- Linking eflob environments to a cloud account.
...
```sh
vp run --filter eflob-relay deploy
```
...
```sh
vp run --filter eflob-relay deploy -- --stage prod
vp run --filter eflob-relay deploy -- --env-file .env.local
```
...
- [T3 Connect Architecture Overview](../../docs/cloud/eflob-connect-auth-flow.html) for the full link,
