...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    EFLOB_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    EFLOB_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    EFLOB_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    EFLOB_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    EFLOB_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    EFLOB_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("EFLOB_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("EFLOB_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "eflobRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the eflob relay through Alchemy."));
