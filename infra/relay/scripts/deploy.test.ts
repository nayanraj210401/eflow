...
describe("reconcileRootEnvRelayUrl", () => {
  ...
  it("preserves unrelated root env entries while replacing a previous relay URL", () => {
    expect(
      reconcileRootEnvRelayUrl(
        "EFLOB_CLERK_PUBLISHABLE_KEY=pk_test_example\nEFLOB_RELAY_URL=https://old.example.test\n",
...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "eflob-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "eflob-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "EFLOB_MOBILE_OTLP_TRACES_DATASET=eflob-mobile-traces-dev",
        "EFLOB_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "EFLOB_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "EFLOB_RELAY_CLIENT_OTLP_TRACES_DATASET=eflob-relay-client-traces-dev",
        "EFLOB_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
    ...
    expect(
      reconcileRootEnvPublicConfig(
        [
          "EFLOB_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "EFLOB_RELAY_URL=https://old.example.test",
          "EFLOB_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "EFLOB_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "EFLOB_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "EFLOB_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "EFLOB_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "EFLOB_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "EFLOB_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "EFLOB_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "EFLOB_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
