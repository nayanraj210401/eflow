...
export const T3_PROJECT_FILE_NAME = "eflob.json";

/** Public URL of the published JSON Schema for {@link T3ProjectFile}. */
export const T3_PROJECT_FILE_SCHEMA_URL = "https://eflob.dev/schema/eflob.json";
...
export const T3ProjectFileScript = Schema.Struct({
  name: trimmedNonEmpty({
    description: "Display name for the script, shown in the eflob scripts menu.",
  }),
  command: trimmedNonEmpty({
    description: "Shell command executed in a eflob terminal at the project root.",
  }),
  ...
  description: "A project script that team members can import into eflob.",
});
...
export const T3ProjectFile = Schema.Struct({
  ...
  iconPath: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          'Workspace-relative path to the project icon (e.g. "assets/logo.svg"). Checked before eflob\'s built-in icon locations.',
      },
  ...
  scripts: Schema.optionalKey(
    Schema.Array(T3ProjectFileScript)
      .annotate({
        description: "Project scripts shared with everyone who opens this repository in eflob.",
      })
  ...
  description:
    "Checked-in project configuration for eflob (t3.json at the repository root). See https://eflob.dev for documentation.",
});
