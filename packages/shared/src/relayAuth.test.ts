...
describe("Clerk relay auth", () => {
  it("derives a custom Frontend API hostname from a Clerk publishable key", () => {
    expect(clerkFrontendApiHostnameFromPublishableKey(clerkPublishableKey("clerk.eflob.dev"))).toBe(
      "clerk.eflob.dev",
    );
    expect(clerkFrontendApiUrlFromPublishableKey(clerkPublishableKey("clerk.eflob.dev"))).toBe(
  ...
  it("reports semantic frontend API failures without inventing a cause", () => {
    ...
    const pathFrontendApi = "clerk.eflob.dev/path";
  ...
  it("allows standard Clerk hosts and an exact configured custom hostname", () => {
    ...
    expect(isAllowedClerkFrontendApiHostname("clerk.eflob.dev", "clerk.eflob.dev")).toBe(true);
    expect(isAllowedClerkFrontendApiHostname("attacker.example", "clerk.eflob.dev")).toBe(false);
    expect(isAllowedClerkFrontendApiHostname("nested.clerk.eflob.dev", "clerk.eflob.dev")).toBe(
      false,
    );
  });
});
