...
- users can generate a hosted pairing link for `app.eflob.dev`
...
- `https://app.eflob.dev` cannot reliably call `http://...` or `ws://...` endpoints because browsers block mixed content.
...
- Generate `app.eflob.dev` links only from endpoints that are likely hosted-browser compatible, or show a warning with an explicit fallback.
...
```text
https://app.eflob.dev/pair?host=<encoded endpoint httpBaseUrl>#token=<one-time token>
```
...
- HTTP private-network endpoints are marked incompatible with `app.eflob.dev`.
...
- `app.eflob.dev` works as a real client shell.
...
- `app.eflob.dev` can pair a reachable HTTPS backend and reconnect after reload.
- A saved environment can be used without any backend at `app.eflob.dev`.
...
The tunnel fabric can later be Pipenet-derived, Tailscale-derived, or another reverse tunnel implementation. The rest of eflob should only see an `AdvertisedEndpoint`.
